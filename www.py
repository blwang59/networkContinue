...
# s=time.clock()
driver.get("https://academic.microsoft.com/#/search?iq=And(Ty%3D'0'%2CComposite(C.CId%3D1135342153))&q=papers%20in%20conference%20www&filters=&from=2272&sort=1")
print('get www')
...
def get_content(d, mode):
    ...
    for b in button:
        if pattern.match(b.text):
            b.click()
        # webdriver.support.expected_conditions.element_to_be_clickable(b)
        # if b.is_enabled():
        #     b.click()
    ...
    with codecs.open('./tests/acdatas_WWW.csv',mode, encoding='utf-8') as f:

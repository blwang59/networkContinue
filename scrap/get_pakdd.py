...
# s=time.clock()
driver.get("https://academic.microsoft.com/#/search?iq=And(Ty%3D'0'%2CComposite(C.CId%3D1178327129))&q=papers%20in%20conference%20pakdd&filters=&from=48&sort=1")
print('get pakdd')
...
def get_content(d, mode):
    ...
    with codecs.open('./results/acdatas_PAKDD.csv', mode, encoding='utf-8') as f:
        f_csv = csv.DictWriter(f, headers)
        # f_csv.writeheader()
        f_csv.writerows(rows)


for i in range(56,933,8):
    driver.get("https://academic.microsoft.com/#/search?iq=And(Ty%3D'0'%2CComposite(C.CId%3D1134284382))&q=papers%20in%20conference%20recsys&filters=&from="+str(i)+"&sort=1")
# while True:
#     try:
